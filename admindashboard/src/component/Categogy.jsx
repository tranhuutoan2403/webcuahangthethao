...
const Categogy = () => {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Bạn có chắc muốn xóa loại sản phẩm này?")) {
      fetch(`http://localhost:5000/api/categogy/${id}`, { method: "DELETE" })
        .then(res => {
          if (res.ok) {
  ...
  return (
    <div className="user-list">
      <div className="top-actions">
        <button className="edit-btn" onClick={() => navigate("/categogy/add")}>
          Thêm
        </button>
      </div>
            ...
            categories.map(category => (
              <tr key={category.category_id}>
                ...
                <td>
                  <button
                    className="edit-btn"
                    onClick={() => navigate(`/categogy/update/${category.category_id}`)}
                  >
                    Sửa
                  </button>
                  <button
