...
const CategoryBrandPage = () => {
  const { categorySlug, brandSlug } = useParams(); // ✅ Lấy slug từ URL
  const [products, setProducts] = useState([]);
  const [brandName, setBrandName] = useState("");
  const [flashSales, setFlashSales] = useState([]);
  const [timer, setTimer] = useState({});
  const [categoryName, setCategoryName] = useState("");
// ✅ Lấy tên danh mục trong bảng category
  useEffect(() => {
  ...
  // ✅ Lấy danh sách sản phẩm theo category + brand
  useEffect(() => {
    fetch(`http://localhost:5000/api/products/${categorySlug}/${brandSlug}`)
      .then((res) => res.json())
